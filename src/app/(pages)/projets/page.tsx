        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <Link
              ...
              className="group relative flex flex-col rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 transition hover:border-violet-600/50 hover:bg-zinc-900"
            >
              {/* Category badge */}
              <span className="mb-4 inline-block w-fit rounded-full border border-zinc-700 bg-zinc-800 px-3 py-0.5 text-xs font-medium text-zinc-300">
                {categoryLabel[project.category] ?? project.category}
              </span>

              {/* Year + title */}
              <span className="mb-1 text-xs text-zinc-500">{project.year}</span>
              <h3 className="text-xl font-semibold text-zinc-100 transition group-hover:text-violet-400">
                {project.title}
              </h3>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                {project.tags.length > 4 && (
                  <span className="rounded-md bg-zinc-800 px-2.5 py-0.5 text-xs text-zinc-500">
                    +{project.tags.length - 4}
                  </span>
                )}
              </div>
